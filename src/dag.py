...
def create_example_circuit() -> QuantumDAG:
    """
    Create an example quantum circuit DAG for testing.
    ...
    # Layer 1
    dag.add_gate('CNOT', [0, 1])
    dag.add_gate('CNOT', [2, 3])
    
    # Layer 2
    dag.add_gate('RZ', [1], parameters={'angle': np.pi/4})
    dag.add_gate('RZ', [3], parameters={'angle': np.pi/4})
    
    # Layer 3
    dag.add_gate('CNOT', [1, 2])
    
    # Layer 4
    dag.add_gate('MEASURE', [0])
    dag.add_gate('MEASURE', [1])
    dag.add_gate('MEASURE', [2])
    dag.add_gate('MEASURE', [3])
    
    return dag


def create_distributed_example() -> QuantumDAG:
    """
    Create an example distributed quantum circuit.
    
    Returns:
        Example QuantumDAG with processor assignments
    """
    dag = create_example_circuit()
    
    # Assign gates to processors
    # Processor 0: qubits 0-1
    # Processor 1: qubits 2-3
    dag.partition_by_qubits([[0, 1], [2, 3]])
...
if __name__ == "__main__":
    # Example usage
    print("Creating example quantum circuit DAG...")
    dag = create_distributed_example()
    ...
    dag.draw(layout='layered', show_processors=True, save_path='dag_visualization.png')
    
    # Example: Topological sort
    print("\nTopological order:")
    for gate_id in dag.topological_sort():
        gate = dag.gates[gate_id]
        print(f"  {gate}")
    
    # Example: Cross-partition gates
    print("\nCross-partition gates (require teleportation):")
    for gate_id in dag.get_cross_partition_gates():
        gate = dag.gates[gate_id]
