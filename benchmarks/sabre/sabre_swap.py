...
# print('SabreSWAP qiskit 0.33.0', DECAY_RATE)
...
class SabreSwap(TransformationPass):
    ...
    **References:**
    ...
    def __init__(self, coupling_map, heuristic="basic", seed=None, fake_run=False):
        ...
        self.EXTENDED_SET_SIZE = self.coupling_map.size()
    ...
    def run(self, dag):
        ...
        # print(f"The front layer has {len(front_layer)} gates: \n    {front_layer}")
        while front_layer:
            execute_gate_list = []
            ...
            #     self.heuristic, front_layer, extended_set, current_layout, swap_qubits=None
            #     )
            # print(f"\n The previous H score is  {prescore}")
            ...
            #sl 230226
            if self.pre_h_score != None and min_score >= self.pre_h_score:
                print('The SabreQ H-score does not decrease!', min_score, self.pre_h_score)  
                # raise Exception('The SabreQ H-score increased!', round(min_score,2), round(self.pre_h_score,2))  
            self.pre_h_score = min_score
            ...
            # print(f" The selected SWAP is {best_swap}")
        ...
        # print('sabre0330test', current_layout)

        if not self.fake_run:
            # print('sabre0330', current_layout)
            return mapped_dag
        return dag
    ...
    def _obtain_extended_set(self, dag, front_layer):
        ...
        while tmp_front_layer and not done:
            ...
            for node in tmp_front_layer:
                ...
                if len(extended_set) >= self.EXTENDED_SET_SIZE: #sl
                    done = True
                    break
